...
function Assert-FfmpegFeature {
    ...
    $commandContent = (($content -split "`r?`n") | Where-Object {
        $_ -notmatch '^\s*(?:REM(?:\s|$)|::|#)'
    }) -join "`n"
    $escapedFeature = [regex]::Escape($Feature)
    $pattern = "(?im)ffmpeg\[[^\]\r\n]*\b$escapedFeature\b[^\]\r\n]*\]"
    $matches = [regex]::Matches($commandContent, $pattern)
...
Assert-FfmpegFeature -RelativePath 'build.bat' -Feature 'amf' -MinimumMatches 2 -MaximumMatches 2
Assert-FfmpegFeature -RelativePath 'setup.bat' -Feature 'amf' -MinimumMatches 1 -MaximumMatches 1
Assert-FfmpegFeature -RelativePath '.github/workflows/selftest.yml' -Feature 'amf' -MinimumMatches 1 -MaximumMatches 1
# Classic intentionally omits an AV1 software fallback; Modern is the sole
# build.bat edition that requests AOM. Both editions still request AMF so a
# supported Radeon can expose hardware AV1 at runtime.
Assert-FfmpegFeature -RelativePath 'build.bat' -Feature 'aom' -MinimumMatches 1 -MaximumMatches 1
Assert-FfmpegFeature -RelativePath 'setup.bat' -Feature 'aom' -MinimumMatches 1 -MaximumMatches 1
Assert-FfmpegFeature -RelativePath '.github/workflows/selftest.yml' -Feature 'aom' -MinimumMatches 1 -MaximumMatches 1
