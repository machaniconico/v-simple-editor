...
QList<PlanarTrackerPreset> Registry::allPresets() const {
    QList<PlanarTrackerPreset> all;
    for (const auto& p : builtinPresets()) all.append(p);
    all.append(m_userPresets);
    return all;
}

QList<PlanarTrackerPreset> Registry::userPresets() const {
    return m_userPresets;
}

std::optional<PlanarTrackerPreset> Registry::findByDisplayName(const QString& name) const {
...
bool Registry::saveUserPreset(const PlanarTrackerPreset& p) {
    ...
    settings.beginGroup("planarTrackerPresets/userPresets");
