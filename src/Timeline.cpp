...
void Timeline::setupUI()
{
    ...
    m_markerLane = new MarkerLane(tracksContainer);
    m_markerLane->setMarkers(&m_markersData);
    ...
    connect(m_scrollArea->horizontalScrollBar(), &QScrollBar::valueChanged,
            this, [this](int v){ if (m_markerLane) m_markerLane->setScrollOffset(v); });
...
void Timeline::setZoomLevel(double pixelsPerSecond)
{
    ...
    if (m_textStrip)
        m_textStrip->setPixelsPerSecond(m_zoomLevel);
    if (m_markerLane) m_markerLane->setPixelsPerSecond(m_zoomLevel);
...
MarkerLane::MarkerLane(QWidget *parent) : QWidget(parent)
{
    setFixedHeight(16);
    setStyleSheet("background-color: #1e1e1e;");
}

void MarkerLane::setMarkers(const QVector<Marker> *markers)
{
    m_markers = markers;
    update();
}
...
void MarkerLane::setScrollOffset(int v)
{
    m_scrollOffset = v;
    update();
}
...
void MarkerLane::paintEvent(QPaintEvent *)
{
    ...
    if (!m_markers || m_pps <= 0.0)
        return;
    for (const Marker &m : *m_markers) {
        const double sec = m.timelineUs / 1'000'000.0;
        const int x = static_cast<int>(sec * m_pps) - m_scrollOffset;
        if (x < -2 || x > width() + 2)
            continue;
        p.setPen(m.color.isValid() ? m.color : QColor(255, 80, 80));
        ...
        if (!m.label.isEmpty()) {
            ...
            f.setPointSize(7);
            p.setFont(f);
            p.drawText(x + 2, height() - 2, m.label);
        }
    }
}
...
{
    for (const Marker &m : m_markersData)
        if (m.id == id) return m;
    return Marker{};
}
...
QVector<Marker> Timeline::markersInRange(qint64 startUs, qint64 endUs) const
{
    ...
    for (const Marker &m : m_markersData)
        if (m.timelineUs >= startUs && m.timelineUs <= endUs) result.append(m);
    return result;
}
...
{
    for (const Marker &m : m_markersData)
        if (m.timelineUs > timelineUs) return m.id;
    return -1;
}
...
int Timeline::prevMarkerBefore(qint64 timelineUs) const
{
    int id = -1;
    for (const Marker &m : m_markersData) {
        if (m.timelineUs < timelineUs) id = m.id;
        else break;
    }
    return id;
}
