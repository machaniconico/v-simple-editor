# NLE parity + libavcore + tracker presets + selftest SSOT + selftest split + 配信認証基盤 + clip-extractor + コマンドパレット + bug fix (52 commits)

base: `main` ← head: `feat/trackmatte-ssot-parity`
範囲: **52 commits / ~150 files / +56,000 / -20,000** (概算)

## Summary

`main` から 52 commits 分の改修を 1 PR でまとめる。コアテーマは 14 つ。

MSVC Release build green。回帰 selftest 14/14 PASS、`--selftest=list` = 69 entry。
...
### 11. 配信認証基盤 Phase 5A〜5J — 1 commit (f07c1e9 に含む)

5 platform (YouTube / Vimeo / Instagram / X / Twitch) の認証・トークン管理を統一基盤に移行:

- **CredentialStore**: 4-tier 解決 (env→Vault→QSettings→default) + access_token TTL (expiresAt) helper
- **CredentialVault**: Windows Credential Manager (`wincred.h`) 統合 — 暗号化永続化 + auto-migrate
- **CredentialDialog**: 5 platform 統一認証 UI (QDialog、QSettings → Vault 自動移行)
- **OAuth mock harness**: baseUrl config injection (4 platform、backward-compatible) + QTcpServer 6 route mock server (621 行) + refresh_token auto-trigger (`refreshIfExpired()`)
- **CredentialAuditLog**: JSONL format (1 行 1 JSON `ts/type/platform/key/message`) + `readEntries()` + `purgeOlderThanDays(30)` 起動時自動配線 (YouTube / Vimeo OAuth)
- 新規 selftest: `credential-store` (10 gate) / `credential-vault` (10 gate) / `cred-ttl` (8 gate) / `oauth-mock-e2e` (10 gate) / `oauth-refresh-e2e` (10 gate) / `platform-mock-e2e` (13 gate) / `cred-audit-log` (10 gate)

### 12. Phase 6 clip-extractor full pipeline import 6A〜6F — 1 commit (f07c1e9 に含む)

外部ツール (yt-dlp / Whisper / LLM / Premiere XML / YouTube chapter) を統合したクリップ抽出パイプラインを新規追加:

- **6A yt-dlp**: `YtdlpDownloader` (QProcess wrapper) + `YtdlpDownloadDialog` + MainWindow「URL から動画を取り込み」メニュー
- **6B Whisper**: `WhisperTranscriber` (subprocess wrapper) + `WhisperTranscribeDialog` + `src/libavcore/AudioExtract.{cpp,h}` (WAV 抽出)
- **6C transcript highlight**: `TranscriptHighlighter` (LLM ベース highlight 検出) + `TranscriptHighlightDialog` + MainWindow 統合
- **6D auto clip**: `AutoClipGenerator` (highlight → clip segment 生成) + `AutoClipDialog`
- **6E Premiere XML**: `PremiereXmlExporter` (clip segment → Premiere Pro XML) + `ExportDialog` 統合
- **6F YouTube chapter**: `YoutubeChapterGen` (transcript → chapter マーカー生成) + `ExportDialog` 「YouTube 概要欄チャプター生成」checkbox
- 新規 selftest: `whisper-transcribe` / `transcript-highlighter` / `auto-clip-gen` / `premiere-xml` / `youtube-chapter` / `ytdlp-downloader`

### 13. コマンドパレット (機能検索) — 1 commit (f07c1e9 に含む)

VS Code 風のコマンドパレットを新規追加:

- **`CommandSearch`**: 純粋ランキング関数 (前方一致→部分一致→スコア降順)
- **`CommandPaletteDialog`**: live filter + キーボード選択 + 選択即実行 (日本語 UI)
- **MainWindow 統合**: 全メニューアクションを起動時に index 化 + Ctrl+Shift+P ショートカット配線
- 新規 selftest: `command-search` (6 gate)

### 14. fullralph バグチェック — genuine bug 修正 4 件 — 1 commit (a5ff3bc)

8 サブシステム並列レビュー → 独立検証で genuine と確定した実バグを修正:

- **[high] `XVideoUpload::doStatusPoll`**: 空/非終端 state での無限再帰 → attempt 上限 60 で打ち切り
- **[medium] `PremiereXmlExporter`**: 同名 highlight の XML 上書き (データ損失) → index prefix `"001-"` でユニーク化
- **[low] `CredentialStore::set`**: Vault store 失敗時に stale vault が新値を shadow → fallback 前に `erase()` 追加
- **[low] `YtdlpDownloader::cancel`**: `finished` 二重発火 + cancel 直後 restart 競合 → deferred-emit 設計に変更

Codex critic 全レビュー PASS。
...
8ccd7f5 docs(pr): PR_DESCRIPTION.md Known MINOR を現状に同期
f07c1e9 feat(auth+clip+cmd): 配信認証基盤 Phase5A-5J + Phase6 clip-extractor + コマンドパレット
b9eddd4 docs(pr): PR_DESCRIPTION.md を 47 commits / 10 themes 反映に同期 (旧)
a5ff3bc fix(bugs): fullralph genuine bug 修正 4 件 (XVideoUpload/PremiereXml/CredentialStore/YtdlpDownloader)
```

## 新規モジュール / ファイル

- **`src/libavcore/`** (8 ファイル): Concat / Decode / Encode / Probe / VideoFilterGraph + `AudioExtract.{cpp,h}` (Phase 6B 新規)
...
- **`src/util/`** (4 ファイル、151 行): Logger.{h,cpp} / CrashHandler.{h,cpp}
- **Phase 5 認証基盤**: `CredentialStore.{cpp,h}` / `CredentialVault.{cpp,h}` / `CredentialAuditLog.{cpp,h}`
- **Phase 6 clip-extractor**: `YtdlpDownloader.{cpp,h}` / `YtdlpDownloadDialog.{cpp,h}` / `WhisperTranscriber.{cpp,h}` / `WhisperTranscribeDialog.{cpp,h}` / `TranscriptHighlighter.{cpp,h}` / `TranscriptHighlightDialog.{cpp,h}` / `AutoClipGenerator.{cpp,h}` / `AutoClipDialog.{cpp,h}` / `PremiereXmlExporter.{cpp,h}` / `YoutubeChapterGen.{cpp,h}`
- **コマンドパレット**: `CommandSearch.{cpp,h}` / `CommandPaletteDialog.{cpp,h}`
...
```bash
./build_win/Release/v-simple-editor.exe --selftest=list   # 69 entry 列挙
...
| `--selftest=credential-store` | 10/10 PASS |
| `--selftest=credential-vault` | 10/10 PASS |
| `--selftest=cred-ttl` | 8/8 PASS |
| `--selftest=oauth-mock-e2e` | 10/10 PASS |
| `--selftest=oauth-refresh-e2e` | 10/10 PASS |
| `--selftest=platform-mock-e2e` | 13/13 PASS |
| `--selftest=cred-audit-log` | 10/10 PASS |
| `--selftest=premiere-xml` | 4/4 PASS |
| `--selftest=youtube-chapter` | 3/3 PASS |
| `--selftest=command-search` | 6/6 PASS |
| `--selftest-trackmatte-rm6-duplicate` (legacy argv) | RM-6 PASS |
| `--selftest-trackmatte-rm5-reorder` (legacy argv) | RM-5 PASS |
| その他 ~45 種 (vfx/pro/mograph/easing/chroma/loudness/watermark/youtube 等) | 全 exit 0 |
...
- [ ] `./build_win/Release/v-simple-editor.exe --selftest=list` で 69 行出力 → exit 0
- [ ] 主要 selftest を `--selftest=<name>` 形式で起動 → 全 exit 0
- [ ] `--selftest=textexport` で `TEXTEXPORT mean MSE` > 20 + `TEXTEXPORT selftest OK` → exit 0
- [ ] `--selftest=credential-store` / `credential-vault` / `cred-audit-log` → 各 10/10 PASS
- [ ] `--selftest=oauth-mock-e2e` / `oauth-refresh-e2e` / `platform-mock-e2e` → 全 PASS
- [ ] `--selftest=premiere-xml` / `youtube-chapter` / `command-search` → 全 PASS
...
- [ ] GUI 起動 → Ctrl+Shift+P でコマンドパレット → live filter + 選択実行 OK
- [ ] GUI 起動 → 「URL から動画を取り込み」メニュー → YtdlpDownloadDialog 表示 OK
...
- 配信認証基盤 Phase 5A〜5J: dualralph/fullralph Codex gpt-5.4 各 PRD 完走 + 全 selftest PASS + 回帰 5 種 exit 0
- Phase 6 clip-extractor: fullralph Codex (6E/6F selftest) + Claude direct (6A-3/6F-2 UI) 完走、MSVC build green
- コマンドパレット: fullralph 5 story 完走、command-search 6/6 PASS
- genuine bug 修正 4 件: 8 サブシステム並列レビュー → 独立検証で確定 → Codex critic 全 PASS (`a5ff3bc`)
...
```bash
# 1. push 状態: ローカル HEAD が origin/feat/trackmatte-ssot-parity から 52 commits ahead (未 push)
...
#   Title: "NLE parity + libavcore + tracker presets + selftest SSOT + 配信認証基盤 + clip-extractor + コマンドパレット (52 commits)"
