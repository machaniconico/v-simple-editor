# NLE parity + libavcore + tracker presets + selftest SSOT + CI draft + docs (36 commits)

base: `main` ← head: `feat/trackmatte-ssot-parity`
範囲: **36 commits / 86 files / +27,007 / -7,686**

## Summary

`main` から 36 commits 分の改修を 1 PR でまとめる。コアテーマは 8 つ。
...
### 5. Documentation / cleanup — 3 commits
...
### 6. selftest argv-switch 統一シリーズ — 4 commits
...
- `src/main.cpp` の argv-switch dispatch (anonymous namespace 内 `kArgvSelftests[]` table)
...
- [ ] `./build_win/Release/v-simple-editor.exe --selftest=list` で 54 entry 出力 → exit 0
...
```bash
# 1. push (deny ルールでエージェントからは push できないので手動)
git push origin feat/trackmatte-ssot-parity

# 2a. gh CLI を使う場合 (現環境 gh 未導入):
gh pr create \
  --base main \
  --head feat/trackmatte-ssot-parity \
  --title "NLE parity + libavcore + tracker presets + selftest argv-switch unification (28 commits)" \
  --body-file .omc/PR_DESCRIPTION.md

# 2b. Web から PR 作成 (gh 未導入なので推奨):
#   https://github.com/machaniconico/v-simple-editor/compare/main...feat/trackmatte-ssot-parity
